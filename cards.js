const deckID = 'qo1cmqdpgsj5'

// Make a request to the Deck of Cards API to request a single card from a newly shuffled deck. Once you have the card, console.log the value and the suit (e.g. “5 of spades”, “queen of diamonds”).

async function drawCard(){
    const response = await axios.get(`http://deckofcardsapi.com/api/deck/${deckID}/draw/?count=1`)
    const card = response.data.cards[0];
    console.log(`${card.value} of ${card.suit}`)
}

drawCard();

// Make a request to the deck of cards API to request a single card from a newly shuffled deck. Once you have the card, make a request to the same API to get one more card from the same deck.
// Once you have both cards, console.log the values and suits of both cards.

async function drawTwice(){
    const resp1 = await axios.get(`http://deckofcardsapi.com/api/deck/new/draw/?count=1`);
    const deckId = resp1.data.deck_id;
    const cards = resp1.data.cards;

    const resp2 = await axios.get(`http://deckofcardsapi.com/api/deck/${deckId}/draw/?count=1`);
    cards.push(resp2.data.cards[0])
    cards.forEach(card => console.log(`${card.value} of ${card.suit}`))

}

drawTwice();

// Build an HTML page that lets you draw cards from a deck. When the page loads, go to the Deck of Cards API to create a new deck, and show a button on the page that will let you draw a card. Every time you click the button, display a new card, until there are no cards left in the deck.

